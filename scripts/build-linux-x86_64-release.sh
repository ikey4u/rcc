#!/bin/sh
# Build the Linux x86_64 static RCC controller (ELF) with linux/macOS/Windows
# sysroots in the embedded pack. Apple SDK is never packed; set
# RCC_APPLE_SDK_ROOT at runtime.
set -eu
...
resource_actual_sha256=$(sha256sum "$resource_archive" | awk '{print $1}')
...
source_actual_sha256=$(sha256sum "$source_archive" | awk '{print $1}')
...
engine_patch_sha256=$(sha256sum "$engine_patch" | awk '{print $1}')
...
python_executable=${RCC_LLVM_PYTHON:-/usr/bin/python3.8}
...
engine_build_id=llvm-22.1.8-$identity_targets-elf-$identity_build_type-$identity_lto-ca7933e47d3a-patch-$engine_patch_identity
...
"$ninja_command" -C "$llvm_build_directory" -j "$build_jobs" \
    clang lld llvm-ar llvm-ranlib llvm-config llvm-dlltool llvm-windres llvm-nm llvm-lib \
    lib/libLLVMMCA.a lib/libLLVMX86TargetMCA.a lib/libLLVMDTLTO.a

if [ ! -x "$llvm_build_directory/bin/llvm-ranlib" ] && [ -x "$llvm_build_directory/bin/llvm-ar" ]; then
    ln -f "$llvm_build_directory/bin/llvm-ar" "$llvm_build_directory/bin/llvm-ranlib"
fi
if [ ! -x "$llvm_build_directory/bin/ld.lld" ]; then
    echo "ld.lld is missing from $llvm_build_directory/bin" >&2
    exit 65
fi

# Staging (musl/mingw/libc++) must use a Linux-runnable clang 22. Prefer the
# just-built engine clang so we do not depend on the official Linux-X64 glibc.
stage_bootstrap=$llvm_build_directory
if [ -n "$bootstrap_prefix" ] && [ -x "$bootstrap_prefix/bin/clang" ]; then
    stage_bootstrap=$bootstrap_prefix
fi
# gnu staging reads these instead of taking LLVM paths as arguments.
...
if [ -f "$mingw_archive" ]; then
    ...
    echo "skipping windows sysroots: mingw-w64 archive not present" >&2
fi
...
link_clang=${stage_bootstrap}/bin/clang++
if [ ! -x "$link_clang" ]; then
    echo "clang++ missing at $link_clang" >&2
    exit 65
fi
