...
impl EnvironmentManifest {
    ...
    pub fn render_cmake(&self) -> Result<String, EnvironmentError> {
        ...
        let cc = required_tool(target, ToolKind::Cc)?;
        let cxx = required_tool(target, ToolKind::Cxx)?;
        let linker = required_tool(target, ToolKind::Linker)?;
        let ar = required_tool(target, ToolKind::Ar)?;
        let ranlib = required_tool(target, ToolKind::Ranlib)?;
        ...
        write_cmake_context(&mut output, "TARGET", target);
        if let Some(host) = &self.host {
            write_cmake_context(&mut output, "HOST", host);
        }
        write_cmake_set(&mut output, "CMAKE_SYSTEM_NAME", system_name);
        write_cmake_set(&mut output, "CMAKE_SYSTEM_PROCESSOR", &target.arch);
        if let Some(version) = &target.minimum_os {
        ...
        write_cmake_set(&mut output, "CMAKE_FIND_ROOT_PATH", &target.sysroot);
        ...
        if let Some(rc) = target
            .tool(ToolKind::Rc)
            .or_else(|| target.tool(ToolKind::Windres))
        {
...
mod tests {
    ...
    fn cmake_renderer_exposes_apple_sdk_without_duplicate_compiler_flags() {
        ...
        assert!(!cmake.contains("CMAKE_OSX_SYSROOT"));
